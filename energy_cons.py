    ...
    for i in range(24):
        if (ellipse.voxel_db[vc, i + 2].mat != 0):
            func(ellipse.voxel_db[vc, i + 2].pos, ellipse.voxel_db, vc, ellipse.nx, ellipse.ny, ellipse.nz, ellipse.dx,
                 ellipse.voxel_db[vc, i + 2].mat, ssht.k, ssht.HTC)
        ...
        for i in range(vox.n):
            if (vox.dom[i, j, k] == 1):
                if (vox.side_exposed[i, j, k] > 0):
                    Ttv = (vox.Tdom[i, j, k] + (ssht.HTC * (ellipse.dx / 2) / ssht.k) * ssht.Tamb) / (
                            1 + ssht.HTC * (ellipse.dx / 2) / ssht.k)
                    Tsv.append(Ttv)
                    errv.append(Tsa - Ttv)
...
print("Heart rate = ", ssht.Heat_rate)
print("tetra_volume = ", ellipse.tetra_volume)
print("percentage error = ",
      (ssht.Heat_rate * ellipse.tetra_volume - np.sum(q_tetra)) / (ssht.Heat_rate * ellipse.tetra_volume) * 100, "%")
...
print("Voxel heat error percentage =",
      (ssht.Heat_rate * ellipse.voxel_volume - np.sum(q_vox)) / ssht.Heat_rate * ellipse.voxel_volume * 100, "%")
